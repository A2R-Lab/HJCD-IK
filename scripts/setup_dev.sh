...
if [ "${SKIP_APT:-0}" != "1" ] && command -v apt-get >/dev/null 2>&1; then
  echo "[setup] (0/4) system deps (Eigen3, nlohmann-json) via apt ..."
  sudo apt-get install -y --no-install-recommends libeigen3-dev nlohmann-json3-dev \
    || echo "[setup] WARNING: apt install failed; install libeigen3-dev + nlohmann-json3-dev manually"
else
  echo "[setup] (0/4) skipping apt; ensure these are installed: cuda-toolkit, libeigen3-dev, nlohmann-json3-dev"
fi

echo "[setup] (1/4) submodules ..."
bash scripts/bootstrap.sh                       # external/GRiD + nested GRiDCodeGenerator/URDFParser
...
python scripts/generate_grid.py include/test_urdf/panda.urdf -t panda_grasptarget_hand
