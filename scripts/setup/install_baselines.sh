...
# each stage is independently skippable. See docs/source/user_guide/benchmarks/results.rst for what each baseline covers.
#
...
if [ "${SKIP_CUROBO:-0}" != "1" ]; then
  ...
  if MAX_JOBS="${CUROBO_MAX_JOBS:-4}" "$PY" -m pip install -e "$CUROBO_SRC" --no-build-isolation; then
    ...
    echo "[install_baselines] WARNING: cuRobo build failed — see docs/source/user_guide/benchmarks/results.rst. cuRobo column will be absent."
  fi
else
...
# SWIG + KDL + NLopt + urdfdom (-dev). See docs/source/user_guide/benchmarks/results.rst ("TRAC-IK on a non-ROS box").
...
if [ "${SKIP_TRACIK:-0}" != "1" ]; then
  ...
  if _install_tracik_rosfree; then
    ...
    echo "[install_baselines] WARNING: tracikpy failed (needs swig + liborocos-kdl-dev + libnlopt-dev + liburdfdom-dev; see docs/source/user_guide/benchmarks/results.rst). MMD/Table IV will lack ground truth."
  fi
else
